 ...
 * @return
 */
export const convertCssUnit = ( cssValue, target ) => {
  ...
  const supportedUnits = {
    ...
    vmax: value => value / 100 * Math.max(window.innerWidth, window.innerHeight),
    vmin: value => value / 100 * Math.min(window.innerWidth, window.innerHeight),
